...
class TCN_Network(nn.Module):
    ...
    def forward(self, x):
        # Prop input through TCN
        x = self.tcn(x)
        x = x[:, :, -1]  # keep only the last time step of the output
        x = self.fc(x)
        # x = self.sigmo(x)
        return x
...
def train():
    ...
    for ind_i in range(0, k):
        ...
        for (data, targets) in test_loader:
            ...
            x = x.view(batchsize, ARGS.inputdim, -1)  # reshape the input data for TCN
            outputs = model(x)
...
def parse_arguments():
    ...
    parser.add_argument('--hiddenDimSize', type=int, default=200, help='Size of TCN hidden layer')
    parser.add_argument('--numLayers', type=int, default=2, help='Number of TCN layers')
    parser.add_argument('--kernelSize', type=int, default=2, help='Kernel size for TCN')
    parser.add_argument('--batchSize', type=int, default=10, help='Batch size.')
    parser.add_argument('--nEpochs', type=int, default=100, help='Number of training iterations.')
    parser.add_argument('--lr', type=float, default=0.01, help='Learning rate.')
    parser.add_argument('--dropOut', type=float, default=0.5, help='TCN Dropout.')
